...
set -euo pipefail
cd "$(dirname "$0")/.."
# Cron appends this script's output to logs/deploy.log; the marker lets
# healthcheck.sh find the last deploy's entry and its start time.
...
# a stale local file is never uploaded; absent until build-site.sh has built
# it once (it keeps the previous one when a build fails). A closed day is
# cached a year at the edge (s-maxage) but only a minute in a browser
# (max-age): its bytes change only when its database rows do — a VOD import
# replaces the whole day and renumbers attempt_number across the database,
# run numbers get filled in later — and the manifest's sha256 says when. A
# closed day already published as closed with these very bytes is not
# re-uploaded; one published closed with other bytes gets the days/ prefix
# invalidated (one wildcard path, whatever the count) before the new manifest
# goes up, so a failed invalidation leaves the old manifest in place and the
# next deploy sees the change again.
manifest=site/api/v1/manifest.json
days_changed=0
if [ -f "$manifest" ]; then
  # The manifest published last time. No object yet means the first deploy
  # of the feed; any other failure to read it is not a reason to skip the
  # invalidation, so it stops the deploy.
  prev_err=$(mktemp)
  if prev=$(aws s3 cp "s3://${DOMAIN}/api/v1/manifest.json" - --region "$REGION" --only-show-errors 2>"$prev_err"); then
    jq -e .days <<<"$prev" >/dev/null 2>&1 \
      || { echo "the published manifest is not the expected JSON; not deploying" >&2; rm -f "$prev_err"; exit 1; }
  elif grep -qE 'NoSuchKey|\(404\)|does not exist|Not Found' "$prev_err"; then
    echo "no manifest published yet: first deploy of the per-day feed"
    prev='{"days":[]}'
  else
    echo "cannot read the published manifest: $(cat "$prev_err"); not deploying" >&2
    rm -f "$prev_err"; exit 1
  fi
  rm -f "$prev_err"
  while IFS=$'\t' read -r day path closed sha; do
    ...
    if [ "$closed" = true ]; then
      [ "$old" = "$sha" ] && continue
      cc="public, max-age=60, s-maxage=31536000"
    else
...
# The manifest goes last, once everything it points at is in place and the
# invalidation is in.
if [ -f "$manifest" ]; then
