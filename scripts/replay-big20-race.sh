#!/usr/bin/env bash
# Replay a Big 20 race-board broadcast (a full practice run, or the race)
# into a scratch database, one VOD per chain. A clone of replay-arcathlon.sh:
# the race board is the same pane in the same place — twenty games two rows
# each, the game and then its category in brackets — read by the board
# tracker with the race roster. First seen 2026-09-17 ("Big 20 #23 / Practice
# Run").
#
#   ./scripts/replay-big20-race.sh <vod_id> [vod_id...]
#
...
# read off the row.
#
# Per VOD it writes, under $RACE_OUT (default big20-race-db/):
...
#   sqlite3 -header -column "$RACE_OUT/vod-<id>.db" \
#     "select game, final_time_ms/1000 secs, last_timer_ms/1000 total \
#      from runs order by last_timer_ms"
#
# Environment: RACE_OUT (output directory), RACE_FPS (default 1 — the board
# is read once a minute, so more frames buy only a sharper marathon total),
# RACE_BIN (default target/release/ngtwitchtimer), RACE_START (seconds into
...
# before the bot looked and is not recorded), RACE_NICE (default 15, so the
# live bot keeps the box; lower it when nothing is live and the run has to
# finish, raise it never).
set -uo pipefail
cd "$(dirname "$0")/.."
out=${RACE_OUT:-big20-race-db}
fps=${RACE_FPS:-1}
bin=${RACE_BIN:-./target/release/ngtwitchtimer}
start=${RACE_START:-0}
prio=${RACE_NICE:-15}
mkdir -p "$out"
...
# The race board (2026-09-17): the big digits only. The Arcathlon crop,
...
match = ["big 20", "big20", "practice run"]
mode = "board"
...
# has already cd'd to.
roster = "assets/big20-roster.toml"
...
EOF
  echo "=== Big 20 race VOD $id — $(date -Is) ==="
